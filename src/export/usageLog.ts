...
function dayBlock(day: DayUsage, peaks: { w?: number; s?: number } | undefined): string[] {
  ...
  if (day.sdk.tokens > 0) {
    lines.push(`- agent-sdk: ${fmtTokens(day.sdk.tokens)} tokens (~$${day.sdk.usd.toFixed(2)} est. · counts toward subscription limits)`);
  }
...
export function buildMonthlyReport(
  ...
  const L: string[] = [
    ...
    ``,
    `- Plan: ${PLAN_NAMES[setup.plan]} · Agent SDK / \`claude -p\` usage currently counts toward your subscription limits (Anthropic paused the separate SDK credit pool on 2026-06-15)`,
  ...
  if (days.length === 0) {
    ...
    L.push(
      ...
      `- busiest day ${busiest.date} (${fmtTokens(busiest.totalTokens)}) · agent-sdk ~$${tot.sdkUsd.toFixed(2)} est. (counts toward limits)`,
      ``,
      `## Daily log`,
    );
